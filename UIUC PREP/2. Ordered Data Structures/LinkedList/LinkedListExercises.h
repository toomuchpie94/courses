...
void LinkedList<T>::insertOrdered(const T& newData) {
  ...
  //Assign new node to heap
  Node* newNode = new Node(newData);

  //if linked list is empty assign newNode to list
  if(empty()){
    head_ = newNode;
    tail_ = newNode;
  } else{
    if(newNode->data < head_->data){ //if newNode is less than the first value
      newNode->next = head_;
      head_->prev = newNode;
      head_ = newNode;
    } else{
      ...
      if(!current){ //if newNode is greater than all the values
        tail_->next = newNode;
        newNode->prev = tail_;
        tail_ = newNode;
      } else{
...
LinkedList<T> LinkedList<T>::merge(const LinkedList<T>& other) const {
  ...
  while(!left.empty() && !right.empty()){
    if (left.head_->data <= right.head_->data){
      ...
      if (merged.empty()){
        merged.head_ = left.head_;
        merged.tail_ = left.head_;
        left.head_ = left.head_->next;
      } else{
        merged.tail_->next = left.head_;
        left.head_->prev = merged.tail_;
        merged.tail_ = left.head_;
        left.head_ = left.head_->next;
      }
      left.size_ --;
      ...
      if (merged.empty()){
        merged.head_ = right.head_;
        merged.tail_ = right.head_;
        right.head_ = right.head_->next;
      } else{
        merged.tail_->next = right.head_;
        right.head_->prev = merged.tail_;
        merged.tail_ = right.head_;
        right.head_ = right.head_->next;
      }
      right.size_ --;
